...
sub read_and_analyse {
	...
	print FILE 'page name'.T.'user'.T.'bytes'.T.'date'.T.'minor'.T.'revertion'.T.'comment'.T.'typo/mistake/error/wrong'.T.'bot'.T.'Rev'.N;
	my $omerta=0;
	my @gods=<DATA>;
	chomp(@gods);
	
	foreach (@data) {
		if ($omerta==1) {$_->[9]='Revered'; $omerta=0} elsif ($_->[3]) {$omerta=1;$_->[9]='Reversion';}
