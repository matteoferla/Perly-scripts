...
@Todo=qw(Aae Aca Afu Ala Ape Asp Atu Bsu Cac Cau Cje Cpn Csa Csy Dra Eco Fnu Gth Hbs homo Hwa Kcr Mac Mae Mbu Mja Mka Mpu Mru Mth Mtu Nap Neq Nos Pab Pae Par Pfu Pho Pin Pmu Pox Pya Rpr Rso Rxy Sau Sce Sso Sty Syn Tac Taq Tga Tma Tpa Tth Tvo Uur);	
#
##################################
...
$file=">special.txt";	
open(SNARE,$file) 	or die "Can't create file: $!";
...
for $Phoenix (0..$#Org) {
	...
	$who= $Org[$x][1];
			...
			$species=$who;
	if ($who eq "nothing") {goto VORTEX;}
	
	$file='genomes/overview/'.$emprefix.'.txt';
	open(ID,$file) or die "Can't open the amino acid file: $!";
	@ancillary=<ID>;
	chomp(@ancillary);
	$ancillary[0]=~ m/Lineage\:(.*)$/gi;
	$lin=$1;
	@lineus=split(/\; /,$lin);
	$what=$lineus[0]."\>".$lineus[1];
	
	
	print "\n\n".(int(1000*$Whom/$#Org)/10)." percent done.\n$species\n($what)\n";
	...
	$Ultimus[0][$Whom]="$what ($species)";
	$Exodus[0][$Whom]="$what ($species)";
	##########	
	
	$file='genomes/fasta/'.$emprefix.'.txt';
	...
	# cross ref keys /cog
	#Start	End	Strand	Length	Gi	GeneID	Locus	Locus_tag	COG(s)
	if ($disable !~ m/X/msg) {
		print "Cross referencing with other indices...\n";
		$file='genomes/table/'.$emprefix.'.txt';
	...
	for $ante (0,2..8,10..13,15..19,21,22,24,25) {
		for ($n=0;$n<=$#gene;$n++) {
			...
			$distcompo[$ante][int(abs($compo[$ante][$n]*100))]+=1/$tg; #one percent increases 0 to 1
		}
			...
			for ($mill = 0; $mill <= $#{$aa[$n]}; $mill++)
			{
				...
				if ($aa[$n][$mill] !~ "X") {
					...
					if ($multiloop > 9) {
						...
						print SNARE ("$who\t$gene[$n][0]\t$gene[$n][1]\t$gene[$n][2]\t".($multiloop+2)."\t$aa[$n][$mill]\n");
	...
	print AAOUT ("local index\tkey\tdescription\tsequence\tmonomer\tuseless\tStart\tEnd\tStrand\tLength\tGi\tGeneID\tLocus\tLocus_tag\tCOG(s)\t\t\t\t\t\t\t\t\t\ttandem pairs\ttandem triplets\ttandem quadruplets\ttandem quintuplets\ttandem hexaplets\ttandem heptaplet\ttandem octuplets\ttandem nonaplets\ttandem plusdecaplet\t".join("\t",@Fullabc)."\n");
	for $i ( 0 .. $#gene ) {
