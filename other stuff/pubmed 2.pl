...
################SETTINGS##########################################################################################
############################################################################################################
my @terms=qw(remediat degrad pesticide toxi pollut enviro soil recombinant organo atrazine glyphosate bioavailab);
...
for $index ($go..$Count) {
	if ($index % 5 == 0) {
		$now=time()-$timer;
		$est=$now*$Count/(0.00001+$index)-$now;
		print ("Papers done: $index (".(int($index/$Count*100))."\%) in ");
		printf("%02d:%02d:%02d", int($now / 3600), int(($now % 3600) / 60), int($now % 60));
		printf("Est. time remaining: %02d:%02d:%02d\n", int($est / 3600), int(($est % 3600) / 60), int($est % 60));
	}
	...
	print OUT "\n$index\t$pmid[$index]\t$title[$index]\t$journal[$index]\t$abs[$index]\t$date[$index]\t$author[$index]\t$doi[$index]\t$place[$index]\t$type[$index]";
	for $j (0..$#terms) {
