...
open(OUT,'>out_listmaker(species).txt') or die 'I died making the output file'.N;

print "Sister strains will be eliminated from three lists.\n";
...
foreach my $wight (keys %wighthoss) {
	my $mess=${[split(/_/,$wight)]}[0].'_'.${[split(/_/,$wight)]}[1];
	#print $mess.N;
	push(@{$kinhoss{$mess}},$wight);
}
...
foreach my $kin (sort keys %kinhoss) {
	...
	if ($#{$kinhoss{$kin}}==0) {print OUT $kin.T.'Singleton'.T.$kinhoss{$kin}[0].T.$wighthoss{$kinhoss{$kin}[0]}[0].T.$wighthoss{$kinhoss{$kin}[0]}[1].T.$wighthoss{$kinhoss{$kin}[0]}[2].N;}
		...
		if ($kin=~/pelagibacter/) {print OUT $kin.T.'magic'.T.$kinhoss{$kin}[0].T.$wighthoss{$kinhoss{$kin}[0]}[0].T.$wighthoss{$kinhoss{$kin}[0]}[1].T.$wighthoss{$kinhoss{$kin}[0]}[2].N;}
		elsif ($kin=~/proteobacter/) {print OUT $kin.T.'magic'.T.$kinhoss{$kin}[0].T.$wighthoss{$kinhoss{$kin}[0]}[0].T.$wighthoss{$kinhoss{$kin}[0]}[1].T.$wighthoss{$kinhoss{$kin}[0]}[2].N;}
		elsif ($#wholeones<0) {exit "DEATH: empty values issue with $kin\n\a";}
		else {
		print OUT $kin.T.'one picked'.T.$wholeones[0].T.$wighthoss{$wholeones[0]}[0].T.$wighthoss{$wholeones[0]}[1].T.$wighthoss{$kinhoss{$kin}[0]}[2].N;
		}
	}
